...
클라이언트 프로세스가 연결을 요청하면 호스트 컴퓨터가 포트 번호를 부여한다. 포트 번호는 1024보다 큰 임의의 정수가 된다. 모든 연결은 유일해야 하며, 호스트에 있는 다른 클라이언트 프로세스가 동일한 웹 서버와 통신을 하려면 그 클라이언트는 유니크한 포트 번호를 부여받게 된다. 이 행동은 모든 연결이 유일한 소켓 쌍으로 구성되는 것을 보장한다. 
